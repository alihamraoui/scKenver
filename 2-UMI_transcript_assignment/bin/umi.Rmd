...
df_summary <- df_summary %>% pivot_longer(!tool, names_to = "metrics", values_to = "val") %>%
                mutate(metrics = recode(metrics,
                               "mean_raw" = "UMI detection",
                               "mean_correct" = "UMI correction"))
```

```{r save_summary_df}
write.csv(df_summary, paste0("UMI_correction_metrics.csv"), row.names = F)
```
