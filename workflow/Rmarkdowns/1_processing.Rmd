...
```{r}
protocole <- strsplit(params$protocole, split = "-")[[1]]

data_name  <- protocole[1]
...
```{r sourceables, message=FALSE, warning=FALSE, include=FALSE}
library(ggplot2)
library(tidyverse)
library(patchwork)
library(data.table)
source(paste0(params$benchmark_dir,"/bin/corr-analysis.R"))
```
...
```{r datasets}
load(file = paste0(benchmark_dir,  "/input/illumina_data.Rdata"))
load(file = paste0(benchmark_dir, "/input/", data_name, "_data.Rdata"))
...
<!-- whitelist_cells <- gsub("-1$", "", colnames(Illumina)) -->
<!-- colnames(Illumina) <- whitelist_cells -->

<!-- Nanopore.corr <- tibble::data_frame() -->
<!-- for (pipe in names(Nanopore)){ -->
...
```{r Save}
output_dir <- file.path(paste0("data/"))
Qc.per.cell <- "cc"
total_UMI <- "cc"
saveRDS(Qc.per.cell, file = paste0(output_dir, data_name,"_QC.per.cell.Rdata"))
saveRDS(total_UMI, file =   paste0(output_dir, data_name,"_total_UMI.Rdata"))
```
