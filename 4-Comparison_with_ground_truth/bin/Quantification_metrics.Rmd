...
subtitle: "Transcript_quantification"
author: "Ali Hamraoui"
...
df_summary <- df.RMSE %>% 
              ...
              mutate(metrics = paste("Counts estimation"))

df_summary_clean <- df_summary %>%
  group_by(tool, metrics) %>%
  summarise(val = 1/min(val))

df_summary_clean
```

```{r save_metrics}
write.csv(df_summary_clean, file = "Quantification_metrics.csv", row.names = F)
```
